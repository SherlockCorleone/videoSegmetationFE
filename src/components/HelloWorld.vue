		...
		<div class="body">
			<div class="left">
				...
				<div class="modeSelect" v-show="styleControl.videoShow" >
					<label for="shot">Shot
						<input type="radio" v-model="modeSelected.shot" value="1"/>1
						<input type="radio" v-model="modeSelected.shot" value="2"/>2
					</label>
					<label for="scene">Scene
						<input type="radio" v-model="modeSelected.scene" value="1">1
						<input type="radio" v-model="modeSelected.scene" value="2">2
					</label>
				</div>
				<div class="functionContainer">
					<button class="startAnalysis" v-show="styleControl.videoShow" @click="startAnalysis">开始分析</button>
					<button class="startAnalysis" v-show="styleControl.videoShow" @click="getSubVideo">获取分割视频</button>
			...
			<div class="right">
				<div class="scene1" v-show="styleControl.videoShow" @click="titleCilck(1)">
					<p :class="{titleBox:!styleControl.titleClick1,titleBoxClick:styleControl.titleClick1}">场景边界信息</p>
				</div>
				<div class="sceneContainer" v-show="styleControl.scene">
					<div class="sceneItem" v-for="(item,index) in sceneList" v-if="item.bound_label==1" :key="item.id">
						<h1 class="title">边界{{++index}}:</h1>
						<h1 class="startTime">{{item.start_time}}</h1>
						<h2 class="startFrame">{{item.start_frame}}</h2>
						<h1 class="endTime">{{item.end_time}}</h1>
						<h2 class="endFrame">{{item.end_frame}}</h2>
					</div>
				</div>
				<div class="scene2" v-show="styleControl.videoShow" @click="titleCilck(2)">
					<p :class="{titleBox:!styleControl.titleClick2,titleBoxClick:styleControl.titleClick2}">分割视频列表</p>
				</div>
				<div class="subVideoContainer" v-show="styleControl.subVideo">
					<div class="subVideoItem" v-for="item in subList" :key="item.id">
						<p @click="playSubVideo(''+item.subVideoUrl)">{{ item.subVideoName }}</p>
